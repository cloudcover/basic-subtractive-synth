...
juce::AudioProcessorValueTreeState::ParameterLayout BasicSubtractiveSynthAudioProcessor::createParameterLayout()
{
    ...
    auto osc2OctaveParam = std::make_unique<juce::AudioParameterFloat>(OSC2_OCTAVE_ID, OSC2_OCTAVE_NAME, 0.0f, 4.0f, 2.0f);
    ...
    auto osc3OctaveParam = std::make_unique<juce::AudioParameterFloat>(OSC3_OCTAVE_ID, OSC3_OCTAVE_NAME, 0.0f, 4.0f, 2.0f);
...
void BasicSubtractiveSynthAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i))))
        {
            ...
            float* osc1_octavePtr = (float*) treeState.getRawParameterValue(OSC1_OCTAVE_ID);
            myVoice->setOsc1OctaveOffset(osc1_octavePtr);
            
            float* osc2_octavePtr = (float*) treeState.getRawParameterValue(OSC2_OCTAVE_ID);
            myVoice->setOsc2OctaveOffset(osc2_octavePtr);
            
            float* osc3_octavePtr = (float*) treeState.getRawParameterValue(OSC3_OCTAVE_ID);
